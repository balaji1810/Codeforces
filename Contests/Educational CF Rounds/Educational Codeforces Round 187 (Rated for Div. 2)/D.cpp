...
void solve() {
    ...
    auto b = readvec<int>(m);

    ll lcm_a = 1;
    const ll cap = (ll)maxval + 1;
    for (int x : a) {
        lcm_a = lcm_a / __gcd(lcm_a, (ll)x) * x;
        if (lcm_a >= cap) { lcm_a = cap; break; }
    }
    ...
    vector<bool> anyDivInA(maxval + 1, false);
    ...
    ll A = 0, B = 0, C = 0;
    for (int y : b) {
        bool alice_can = anyDivInA[y];
        bool only_alice = (lcm_a < cap) && (y % lcm_a == 0);
        if (only_alice)
            A++;
        else if (!alice_can)
            B++;
