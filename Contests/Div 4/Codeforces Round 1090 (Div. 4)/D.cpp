...
vector<ll> primes_numbers = {2};

void ensure_primes_count(int need) {
  if (primes_numbers.size() >= need)
    return;

  ll candidate = primes_numbers.back() + 1;
  if (candidate % 2 == 0)
    candidate++;

  while (primes_numbers.size() < need) {
    bool is_prime = true;
    for (ll p : primes_numbers) {
      if (p * p > candidate)
        break;
      if (candidate % p == 0) {
        is_prime = false;
...
void solve() {
    ...
    ensure_primes_count(n + 1);
    for (int i = 0; i < n; i++) {
        ll val = primes_numbers[i] * primes_numbers[i + 1];
        cout << val << ' ';
    }
    cout << nl;
}
